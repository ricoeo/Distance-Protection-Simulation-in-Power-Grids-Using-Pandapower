...
def compute_misjudgment_counts(device_data, boundaries):
    ...
    """
    misjudgment_counts = [0, 0, 0]  # For boundaries between Zone 1-2, Zone 2-3, Zone 3-Out of Zone

    for _, row in device_data.iterrows():
        zone_calculated = row['zone_calculated']
        zone_sensed_img = compute_sensed_impedance(row['r_sensed'], row['angle_sensed']) # recalculate the zone after changing the polygon
...
def finalize_boundaries(boundary_selection_option, original_boundaries):
    ...
    for zone_index in range(len(original_boundaries)):
        ...
        if zone_index == 0:
            ...
            valid_candidates = [b for b in candidate_boundaries if b > final_boundaries[zone_index - 1]]
            if valid_candidates:
                ...
                final_boundaries[zone_index] = max(original_boundaries[zone_index], final_boundaries[zone_index - 1] + 0.1)
        else:
...
def adjust_zone_boundaries(device_data, protection_device, initial_step=0.1, max_iterations=100):
    ...
    for zone_index in range(len(original_boundaries)):
        ...
        original_misjudgement = compute_misjudgment_counts(device_data, original_boundaries)
        ...
        if sum_misjudgment_1[zone_index] < sum_misjudgment_2[zone_index]:
            ...
            current_boundary = sum_misjudgment_1
        else:

...
HV = 110  # High Voltage side in volts
...
def setup_protection_zones(net, excel_file):
    ...
    for idx in distance_protection_data.index:
        if net.line.at[distance_protection_data.at[idx, "first_line_id"], 'in_service'] == False:
            continue
...
def simulate_faults_along_line(net, line_id, affected_devices, fault_line_on_doubleline_flag, interval_km=0.25, plot_powerflow_flag = False):
    ...
    start_geo_x,start_geo_y = net.bus_geodata.at[net.line.at[line_id, 'from_bus'],'x'],net.bus_geodata.at[net.line.at[line_id, 'from_bus'],'y']
    end_geo_x,end_geo_y = net.bus_geodata.at[net.line.at[line_id, 'to_bus'],'x'],net.bus_geodata.at[net.line.at[line_id, 'to_bus'],'y']
    # if the sensed results are different from the calculated results, it is recorded
    # sense_wrong_dict = {}
    if num_faults > 0:

        fault_locations = [interval_km * i for i in range(1, num_faults)] or [line_length / 2]

        for fault_location in fault_locations:
            # Create a temporary bus at fault location
            temp_bus = pp.create_bus(net, vn_kv=HV, type="n", name="fault_bus",geodata= (start_geo_x+(end_geo_x-start_geo_x)*float(fault_location)/line_length,start_geo_y+(end_geo_y-start_geo_y)*float(fault_location)/line_length))
...
def simulate_faults_for_all_lines(net, protection_devices, interval_km=0.25):
    ...
    double_line_pairs = net.line[net.line['in_service']].groupby(['from_bus', 'to_bus']).filter(lambda x: len(x) > 1)
    ...
    for line_id in net.line.index:
        if net.line.at[line_id, 'in_service']:  # Only consider in-service lines
            ...
            device_data = simulate_faults_along_line(net, line_id, affected_devices,fault_line_on_doubleline_flag, interval_km)
...
def plot_short_circuit_results(net, figure_number):
    ...
    for _, info in current_dict.items():
        ...
        ax.arrow(from_coords.x + offset_x, from_coords.y + offset_y, dx, dy,
                 color='#a36c65', width=line_thickness, head_width=line_thickness*4, head_length=line_thickness*4,
